...
- **Input** : liens de playlist Deezer (Spotify en option, voir plus bas).
...
## Import Spotify (optionnel)

Deezer marche sans rien configurer. Pour accepter aussi des **liens de playlist
Spotify publics**, crée une app gratuite sur le dashboard développeur Spotify et
définis deux variables d'environnement côté serveur :
...
Les pistes Spotify sont ensuite matchées sur Deezer (par ISRC, sinon par
recherche) pour récupérer l'extrait jouable. Les joueurs n'ont **pas** besoin de
se connecter : un simple lien public suffit. (Lire les playlists *privées* d'un
utilisateur demanderait un vrai login OAuth par joueur — pas fait ici.)

## Comment marche une partie

1. Un joueur **crée un salon**, les autres **rejoignent** avec le code à 4 lettres.
2. Chacun colle une ou plusieurs playlists dans le lobby.
...
- Login OAuth Spotify/Deezer pour importer ses playlists privées / ses Liked Songs.
...
```
server.js        serveur + Socket.IO + machine à états du jeu
music.js         import playlists (Deezer natif, Spotify optionnel)
public/
  index.html     écrans (accueil, lobby, jeu, reveal, classement)
  app.js         logique client + audio synchronisé
  style.css      thème
```
