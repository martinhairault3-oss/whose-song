...
function renderLobby() {
  ...
  for (const p of st.players) {
    ...
    el.innerHTML = `<span class="dot"></span>${escapeHtml(p.name)}` +
      (p.isHost ? ' <span class="host">hôte</span>' : '') +
...
function renderPlay() {
  ...
  const iAmOwner = false; // le proprietaire n'est jamais revele -> on ne peut pas le savoir cote client
  // (le serveur ignore le vote du proprietaire de toute facon)
